...
void SubStellarPoint::Name::enable(Font* font, glm::vec4 color, float size) {
    ...
    if (m_billboard == nullptr) m_billboard = new TextBillBoard(m_ssp->m_earth->m_scene, font, m_ssp->name, m_ssp->m_pos, m_color, m_size);
    m_enabled = true;
    update();
}
...
void Constellations2::addBorder(BodyGeometry& geom, size_t rindex) {
    ...
    for (auto& part : records[rindex]->parts) {
        ...
        for (size_t i = part.startindex; i < (part.startindex + part.length); i++) {
            constellationparts.back()->polyline->addPoint(geom.getLoc3D({ deg2rad * records[rindex]->points[i].latitude, deg2rad * records[rindex]->points[i].longitude, surface_offset }));
        }
