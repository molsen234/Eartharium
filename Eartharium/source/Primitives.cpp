...
TextBillBoard::TextBillBoard(Scene* scene, Font* font, std::string text, glm::vec3 position, glm::vec4 color, float size) : m_scene(scene), m_font(font), m_text(text), m_pos(position) {
...
void TextBillBoard::update(glm::vec3 position) {
...
void TextBillBoard::changeColorSize(glm::vec4 color, float size) {
...
void GenericPath::generate() {
    //std::cout << "GenericPath curves: " << m_curves.size() << "\n";
    for (auto& c : m_curves) {
        c->generate();
        //std::cout << " Points: " << c->m_points.size() << "\n";
    }
}
...
// - Add a proto for the class at the top of Primitives.h
...
// - Add method definitions in Primitives.cpp
//   o Constructor must:
