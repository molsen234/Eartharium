...
LLD APluto::EclipticCoordinates(double jd_tt) {
    LLD retval{};
    retval.lat = EclipticLatitude(jd_tt);
    retval.lon = EclipticLongitude(jd_tt);
    retval.dst = RadiusVector(jd_tt);
    return retval;
}

// Illumination
double APluto::MagnitudeAA(double r, double Delta) noexcept {
