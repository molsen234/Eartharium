...
double ASaturn::EclipticLongitude(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
double ASaturn::EclipticLatitude(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
double ASaturn::EclipticDistance(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
LLD ASaturn::EclipticCoordinates(double jd_tt, Planetary_Ephemeris eph) {
    LLD retval{};
...
// Illumination
double ASaturn::MagnitudeAA(double r, double Delta, double DeltaU, double B) noexcept {
    // r = Saturn to Sun distance in AU
    // Delta = Saturn to Earth distance in AU
    // DeltaU = The difference between the Saturnicentric longitudes of the Sun and the Earth, measured in the plane o the ring in radians
    // B = The Saturnicentric latitude of the Earth referred to the plane of the ring in radians
    DeltaU *= rad2deg;
    const double sinB{ sin(B) };
    return -8.88 + (5 * log10(r * Delta)) + (0.044 * fabs(DeltaU)) - (2.60 * sin(fabs(B))) + (1.25 * sinB * sinB);
}
double ASaturn::MagnitudeMuller(double r, double Delta, double DeltaU, double B) noexcept {
    // r = Saturn to Sun distance in AU
    // Delta = Saturn to Earth distance in AU
    // DeltaU = The difference between the Saturnicentric longitudes of the Sun and the Earth, measured in the plane o the ring in radians
    // B = The Saturnicentric latitude of the Earth referred to the plane of the ring in radians
