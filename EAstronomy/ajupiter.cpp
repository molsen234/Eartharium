...
double AJupiter::EclipticLongitude(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
double AJupiter::EclipticLatitude(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
double AJupiter::EclipticDistance(double jd_tt, Planetary_Ephemeris eph) noexcept {
...
LLD AJupiter::EclipticCoordinates(double jd_tt, Planetary_Ephemeris eph) {
    LLD retval{};
...
// Illumination
double AJupiter::MagnitudeAA(double r, double Delta, double i) noexcept {
    // r = Jupiter to Sun distance in AU
    // Delta = Jupiter to Earth distance in AU
    // i = Jupiter phase angle in radians
    i *= rad2deg;
    return -9.40 + (5 * log10(r * Delta)) + (0.005 * i);
}
double AJupiter::MagnitudeMuller(double r, double Delta) noexcept {
    // r = Jupiter to Sun distance in AU
    // Delta = Jupiter to Earth distance in AU
